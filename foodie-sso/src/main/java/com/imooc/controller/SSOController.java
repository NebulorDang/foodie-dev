...
public class SSOController {
     ...
     * CAS的统一登录接口
     * 目的：
     *  1.登录后创建用户的全局对话                 -> uniqueToken
     *  2.创建用户群居门票，用于表示在CAS端是否登录  -> userTicket
    ...
    public String doLogin(String username,
        ...
        // 4.生成ticket门票，全局门票代表用户在CAS端登陆过
        String userTicket = UUID.randomUUID().toString().trim();
        // 4.1 用户全局门票需要放入CAS端的cookie中
        setCookie(COOKIE_USER_TICKET, userTicket, response);

        // 5. userTicket关联用户id，并且放入到redis中，代表用户有门票可以在各个景区游玩
        redisOperator.set(REDIS_USER_TICKET + ":" + userTicket, userResult.getId());

        // 6.生成临时票据，回跳到调用端网站，是由CAS端签发的一个一次性的临时ticket
        String tmpTicket = createTmpTicket();

        /**
         * userTicket: 用于表示用户在CAS端的一个登录状态：已经登陆
